import type { Vector3 } from '../vector/index.js'
...
/**
 * A geodetic reading, structurally. Deliberately narrower than the `Location`
 * the wire carries: the projection needs three numbers and has no business
 * knowing what else a device chose to report alongside them.
 */
...
/** How badly a receiver thinks it is lying, per axis. */
export interface Accuracy {
  horizontalAccuracy: number
  verticalAccuracy: number
}

export type GeodeticFix = Geodetic & Accuracy
 ...
 */
export function projectLocation(location: Geodetic, origin: Origin): Vector3 {
  return {
 ...
 */
export function toEcef(location: Geodetic): Vector3 {
 ...
 */
export function unprojectLocation(point: Vector3, origin: Origin, accuracy: Accuracy): GeodeticFix {
  return {
