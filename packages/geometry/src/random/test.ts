...
import { deriveSeed, Random } from './index.js'
...
describe('Random', () => {
  ...
  /** Uniform integers, taken off `float()` so there is no modulo bias to have. */
  it('draws integers below a bound, and every one of them', () => {
    const random = new Random(17)
    const seen = new Set(sample(20_000, () => random.below(6)))

    expect([...seen].sort()).toEqual([0, 1, 2, 3, 4, 5])
    expect(new Random(19).below(1)).toBe(0)
  })
  ...
  it('draws booleans at the requested rate', () => {
    const random = new Random(17)
    const values = sample(200_000, () => (random.bool(0.05) ? 1 : 0))

    expect(mean(values)).toBeCloseTo(0.05, 2)
  })
})
