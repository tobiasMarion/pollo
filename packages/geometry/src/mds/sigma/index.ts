import { median } from '../../stats/index.js'
 ...
 *
 * The *numbers* are not a premise. They are measured, below. Where the estimate
 * starts before it has seen anything is the caller's to choose, because it is a
 * guess about a radio rather than a fact about the maths.
 */
...
function summarize(half: readonly Sample[]) {
  return {
    ...
    sigma: median(half.map(sample => Math.abs(sample.residual))) * MEDIAN_TO_SIGMA,
  }
}
