...
@MainActor @Observable final class EventController {
    enum Phase: String { case locating, searching, empty, ready, connecting, positioning, live, reconnecting, failed }
    ...
    init(location: any LocationSource, ranging: any RangingSource, api: any EventService,
        ...
        self.location = location; self.ranging = ranging; self.api = api
        self.transport = transport; self.light = light; self.deviceId = deviceId
        self.now = now; self.maxPeers = maxPeers
    ...
    func retry() {
        guard active else { return }
        generation += 1; discovery?.cancel(); discovery = nil
        needsDiscovery = true
        phase = .locating; message = "Buscando sua localização…"
        location.start()
    }
    ...
    private func discover(_ value: Location) {
        ...
        phase = .searching; message = "Buscando eventos…"
        ...
        discovery = Task { [weak self] in
            ...
            do {
                let result: Event?
                if let resume { result = try await self.api.event(resume) }
                else { result = try await self.api.nearby(value) }
                guard !Task.isCancelled, current == self.generation, self.active else { return }
                self.event = result?.status == .open ? result : nil
                guard let event = self.event else {
                    self.phase = .empty; self.message = "Nenhum evento por perto."; return
                }
                self.phase = .ready
    ...
    private func teardown() {
        ...
        session = nil
        execute(commands)
        transport.disconnect(); ranging.stop(); location.stop(); light.stop()
        participating = false; brightness = 0; peers = 0
        positioned = false
        diagnostics = nil
    }
    private func input(_ value: SessionInput) {
        ...
        case .joined:
            ...
            message = session.point == nil ? "Aguardando posicionamento…" : "Tudo pronto. Aguarde as luzes."
        case .waiting, .offline:
            phase = .reconnecting; message = "Reconectando…"
        case .stopped:
    ...
    private func execute(_ commands: [SessionCommand]) {
        ...
        for command in commands {
            switch command {
            case .connect: if let event { transport.connect(event.id) }
            case .disconnect: transport.disconnect()
            case .send(let frame): sentFrameCount += 1; transport.send(frame)
            case .beginRanging(let peer): ranging.begin(peer)
            case .configureRanging(let peer, let token): ranging.configure(peer, token: token)
            case .endRanging(let peer): ranging.end(peer)
            }
        }
    }
    private func startFrames() {
        ...
        frames = Task { [weak self] in
            while !Task.isCancelled {
                guard let self, let core = self.session, let cue = core.cue, let point = core.point else { return }
                let elapsed = self.now() - cue.arrivedAt
                let end = effectDelaySeconds(cue.effect, at: point, center: cue.center) + max(cue.effect.activeTime, 0.05)
                self.brightness = core.brightness(at: self.now())
                self.light.render(self.brightness)
                if elapsed >= end { self.brightness = 0; self.light.render(0); return }
