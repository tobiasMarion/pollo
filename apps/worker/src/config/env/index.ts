...
const envSchema = z.object({
  ...
  /**
   * How fast the sensor estimate follows the residuals. A scale recomputed from
   * scratch every tick jitters, and the weights it sets jitter with it — which
   * shows up as the whole crowd breathing.
   */
  WORKER_SCALE_BLEND: z.coerce.number().positive().max(1).default(0.2),

  /**
   * Every nth edge contributes a residual to the sensor estimate. The estimator
   * wants a few hundred samples spread over the distances in play, not the
   * hundred and sixty thousand a full sweep would hand it.
   */
  WORKER_SAMPLING_STRIDE: z.coerce.number().int().positive().default(7),
