...
import { Csr, estimateScale, guttman, Relaxation, type Scale } from '@pollo/geometry'
import { EventGraph } from '../../ingest/graph/index.js'
import { Solver, type SolverOptions } from '../solver/index.js'
 ...
 *
 * Nothing here is imported by `src/` outside tests, and nothing here is built:
 * see `tsconfig.build.json`.
 */

export const INITIAL_SCALE: Scale = { floor: 0.1, relative: 0.05 }

export function solverOptions(overrides: Partial<SolverOptions> = {}): SolverOptions {
  return {
    anchorScale: 1,
    huberKnee: 2,
    sweepsPerTick: 8,
...
/** The real solver, wired the way `composition/` wires it. */
export function solverOf(overrides: Partial<SolverOptions> = {}, alphaMin = 0.2) {
  return new Solver(
    {
      index: new Csr(),
      mean: new Relaxation({ alphaMin }),
      sweep: guttman,
      fitScale: estimateScale,
    },
    solverOptions(overrides),
  )
}
