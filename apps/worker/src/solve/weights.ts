...
import { type Scale, sigmaAt } from './scale.js'
...
export interface AnchorOptions {
   ...
   * wrong together. Independent anchors over-count that agreement, and this is
   * the dial that answers for it until the shared part is estimated outright.
   */
  scale: number
}

/**
 * What a device's own GPS is worth, per axis.
 *
 ...
 * device is held by a dozen measured distances, its own GPS is the worst input
 * in its fit. The trouble is what the anchors are also doing: they are the only
 * thing that fixes the frame. Distances give a shape and say nothing about where
 * that shape sits or which way it faces, so relaxing them lets the whole cloud
 * drift and turn, and it scores worse for a shape that is better. Measured, on a
 * crowd of a hundred and twenty: RMSE went from 2.56 m to 2.93 m. The averaging
 * that fading was reaching for is what `Relaxation` does instead, and it does it
 * without touching the frame.
 */
export function anchorWeights(location: Location, options: AnchorOptions): AnchorWeights {
  const horizontal = Math.max(location.horizontalAccuracy, 0.1)
  const vertical = Math.max(location.verticalAccuracy, 0.1)

  return {
    horizontal: options.scale / (horizontal * horizontal),
    vertical: options.scale / (vertical * vertical),
  }
}
