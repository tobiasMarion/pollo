...
import type { Solver } from './index.js'
...
describe('the temporal step', () => {
  ...
  it('is worth a few per cent once the snapshot is actually solved', () => {
    ...
    const forgetful = run(scenario, solverOf({ sweepsPerTick: 30 }, 1), 120, 900)
    const remembering = run(scenario, solverOf({ sweepsPerTick: 30 }), 120, 900)
  ...
  it('stops paying when the sweep budget is too small to converge', () => {
    ...
    const forgetful = run(scenario, solverOf({ sweepsPerTick: 4 }, 1), 120, 900)
    const remembering = run(scenario, solverOf({ sweepsPerTick: 4 }), 120, 900)
  ...
  it('keeps improving after the crowd has stopped arriving', () => {
    ...
    const early = run(scenario, solverOf(), 12, 901)
    const late = run(scenario, solverOf(), 120, 901)
  ...
  it('follows a device whose owner walked off, after it had settled', () => {
    ...
    const settled = 60
    const { graph, solver } = run(scenario, solverOf(), settled + 40, 902, tick =>
  ...
  it('lets a still crowd keep the evidence it has gathered', () => {
    ...
    const { solver, graph } = run(scenario, solverOf(), windows, 903)
